...
Deno.serve(async (req) => {
    try {
        ...
        const { propertyId, targetLanguages = ['en', 'es', 'fr', 'de'] } = await req.json();
...
País: ${property.country || 'Portugal'}
...
${property.description ? `Descrição base (PT): ${property.description}` : ''}
        `.trim();
        ...
        // Gerar descrição em português se não existir
        if (!property.description) {
            const ptPrompt = `És um especialista em marketing imobiliário em Portugal. Cria uma descrição profissional, atrativa e detalhada para o seguinte imóvel. A descrição deve:
...
Gera apenas a descrição, sem títulos ou formatação adicional.`;
            ...
            const ptResult = await base44.integrations.Core.InvokeLLM({
                prompt: ptPrompt
            });

            descriptions.pt = typeof ptResult === 'string' ? ptResult : ptResult.content || ptResult.message || ptResult.description;
        } else {
            descriptions.pt = property.description;
        }

        // Gerar descrições em cada idioma alvo
        for (const lang of targetLanguages) {
            const langName = languageNames[lang] || lang;
            
            const prompt = `És um especialista em marketing imobiliário internacional. Traduz e adapta a seguinte descrição de imóvel para ${langName}, mantendo o tom profissional e persuasivo. 

IMPORTANTE:
- A tradução deve ser natural e fluente em ${langName}
- Adapta expressões culturais e termos imobiliários para o mercado internacional
- Mantém entre 150-300 palavras
- Preserva o call-to-action mas adapta-o ao idioma
- Usa terminologia imobiliária correta para ${langName}
...
Retorna APENAS a descrição traduzida em ${langName}, sem introduções, títulos ou formatação adicional.`;

            const result = await base44.integrations.Core.InvokeLLM({
                prompt: prompt
            });

            descriptions[lang] = typeof result === 'string' ? result : result.content || result.message || result.description;
        }
