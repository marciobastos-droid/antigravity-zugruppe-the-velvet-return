      ...
      {showAlternatives && convertedPrices.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs text-slate-500">
            ...
            <span>Preços noutras moedas</span>
            {isFallback && <Badge variant="outline" className="text-xs">Estimativa</Badge>}
          </div>
          <div className="grid grid-cols-2 gap-2">
            {convertedPrices.map(({ currency: curr, amount }) => (
              <div key={curr} className="flex justify-between items-center px-3 py-2 bg-slate-50 rounded-lg">
                <span className="text-sm text-slate-600 font-medium">{curr}</span>
                <span className="font-semibold text-slate-900">{formatPrice(amount, curr)}</span>
              </div>
            ))}
          </div>
