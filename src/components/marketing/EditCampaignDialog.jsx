...
import { Loader2, Mail, Facebook, Instagram, Sparkles } from "lucide-react";
import { Checkbox } from "@/components/ui/checkbox";

export default function EditCampaignDialog({ campaign, open, onOpenChange }) {
  ...
  useEffect(() => {
    if (campaign) {
      setFormData({
        ...
        social_ads_config: campaign.social_ads_config || {
          platform: "facebook",
          budget: 0,
          ad_creative: {
            headline: "",
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => base44.entities.MarketingCampaign.update(campaign.id, data),
    onSuccess: () => {
...
Retorna texto otimizado para conversão. Máximo 125 caracteres.`,
  ...
  const handleSubmit = (e) => {
    ...
    updateMutation.mutate(trackingData);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar Campanha de Marketing</DialogTitle>
        </DialogHeader>
