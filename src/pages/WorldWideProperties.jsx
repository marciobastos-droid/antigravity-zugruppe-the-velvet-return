      ...
      <div className="relative bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 text-white py-16 sm:py-24 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold">{t('pages.worldwide.title')}</h1>
          </div>
          <p className="text-xl sm:text-2xl text-blue-100 max-w-3xl">
            {t('pages.worldwide.subtitle')}
          </p>
          <div className="mt-6 flex flex-wrap items-center gap-4 text-blue-200">
            <div className="flex items-center gap-2">
              <Globe className="w-5 h-5" />
              <span className="font-semibold">{countries.length} {t('pages.worldwide.countries')}</span>
            </div>
            <div className="flex items-center gap-2">
              <MapPin className="w-5 h-5" />
              <span className="font-semibold">{properties.length} {t('pages.worldwide.internationalProperties')}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <Card className="mb-8 border-blue-200 bg-gradient-to-r from-blue-50 to-white">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-slate-900">{t('pages.worldwide.filters')}</h3>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">{t('pages.worldwide.country')}</label>
                <Select value={countryFilter} onValueChange={setCountryFilter}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">{t('pages.worldwide.allCountries')}</SelectItem>
              ...
              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">{t('pages.worldwide.businessType')}</label>
                <Select value={listingTypeFilter} onValueChange={setListingTypeFilter}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">{t('pages.worldwide.all')}</SelectItem>
                    <SelectItem value="sale">{t('pages.worldwide.sale')}</SelectItem>
                    <SelectItem value="rent">{t('pages.worldwide.rent')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>

              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">{t('pages.worldwide.propertyType')}</label>
              ...
              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">{t('pages.worldwide.sortBy')}</label>
                <Select value={sortBy} onValueChange={setSortBy}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="newest">{t('pages.worldwide.newest')}</SelectItem>
                    <SelectItem value="price_desc">{t('pages.worldwide.priceHighToLow')}</SelectItem>
                    <SelectItem value="price_asc">{t('pages.worldwide.priceLowToHigh')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between text-sm text-slate-600">
              <span>{t('pages.worldwide.showing')} <strong>{filteredProperties.length}</strong> {t('pages.worldwide.internationalProperties').toLowerCase()}</span>
              {(countryFilter !== "all" || listingTypeFilter !== "all" || propertyTypeFilter !== "all") && (
                ...
                >
                  {t('pages.worldwide.clearFilters')}
                </Button>
              )}
            </div>
          </CardContent>
        ...
        {isLoading ? (
          ...
          <Card className="text-center py-20">
            <CardContent>
              ...
              <h3 className="text-2xl font-bold text-slate-900 mb-2">{t('pages.worldwide.noInternationalFound')}</h3>
              <p className="text-slate-600">{t('pages.worldwide.adjustFilters')}</p>
            </CardContent>
          </Card>
        ) : (
